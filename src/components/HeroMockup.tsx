...
import {
  ...
  Search, Bell, TrendingUp, Zap, RefreshCw, Filter, MoreVertical,
} from 'lucide-react';
          ...
          <div className="flex-1 min-w-0 bg-white">
            <div className="flex items-center justify-between px-4 py-3 border-b border-ink-100">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="w-6 h-6 rounded-full bg-gradient-to-br from-vanta-500 to-accent-500 flex items-center justify-center text-[9px] font-bold text-white">
                  V
                </div>
              </div>
            </div>

            <div className="px-4 py-2.5 flex items-center gap-2 border-b border-ink-100">
              ...
              <div className="hidden md:flex flex-1 items-center gap-1.5 text-[10px] text-ink-400 bg-ink-50 border border-ink-200 rounded-lg px-2.5 py-1.5 ml-2">
                <Search className="w-3 h-3" /> {t('common.search')} in table...
              </div>
              <span className="hidden sm:flex items-center gap-1 badge bg-accent-500 text-white text-[10px] py-1">
            ...
            <table className="w-full text-[11px]">
              <thead>
                <tr className="text-ink-400 border-b border-ink-100">
                  ...
                  <th className="text-left font-medium px-2 py-2 hidden lg:table-cell">Fee</th>
                  ...
                  <tr key={i} className="border-b border-ink-50 hover:bg-vanta-50/40 transition-colors">
                    ...
                    <td className="px-2 py-2.5">
                      <div className="flex items-center gap-1.5">
                        <span className={`w-4 h-4 rounded-full ${r.color} flex items-center justify-center text-white text-[7px] font-bold shrink-0`}>
                          {r.initials}
                        </span>
                        <span className="font-medium text-black whitespace-nowrap">{r.name}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-2 py-2.5 text-ink-500 hidden lg:table-cell whitespace-nowrap">{r.fee}</td>
                    <td className="px-4 py-2.5">
                      <span className={`badge text-[9px] py-0.5 ${statusStyle[r.status]}`}>{r.status}</span>
                    </td>
                    <td className="px-1">
                      <MoreVertical className="w-3 h-3 text-ink-300" />
                    </td>
                  </tr>
                ))}
              </tbody>
